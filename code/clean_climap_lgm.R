...
climap <- read_csv("raw/LGM_CLIMAP.csv")
...
climap <- climap %>% replace_column_name("N. pachyderma s", "N. incompta")
climap <- climap %>% replace_column_name("N. pachyderma d", "N. pachyderma")
...
climap_r <- climap %>% mutate(total = rowSums(across(`O. universa`:`H. digitata`))) %>%
  mutate(across(`O. universa`:`H. digitata`, ~ ./ total)) %>% select(-total)
...
group_and_aggregate(climap_r) %>% write_csv(., "fg/lgm_climap_fg_r.csv")
group_and_aggregate(climap, value_name="Absolute Abundance") %>% write_csv(., "fg/lgm_climap_fg_a.csv")
