...
forcens_r <- melt(forcens,
  id.vars = names(forcens)[1:21],
...
forcens_a <- melt(forcens_a,
  id.vars = names(forcens_a)[1:21],
...
forcens_r %>%
  local_group_and_aggregate("Relative Abundance") %>%
  dplyr::filter(`Relative Abundance` < 1.1) %>%
  fwrite("fg/forcens_fg_r.csv")

forcens_a %>%
  local_group_and_aggregate("Absolute Abundance") %>%
  fwrite("fg/forcens_fg_a.csv")
