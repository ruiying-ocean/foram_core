...
core10 <- core10 %>% select(c("Depth sed [m]", "N. pachyderma s [#/g]")) %>%
...
core9_long <- core9_long %>% group_by(Latitude, Longitude, `depth`, Symbiosis, Spine) %>% 
  summarise_all(.funs = sum, na.rm=T)  %>% ungroup()
core10_long <- core10_long %>% group_by(Latitude, Longitude, `Depth sed [m]`, Symbiosis, Spine) %>%
  summarise_all(.funs = sum, na.rm=T)  %>% ungroup()
core11_long <- core11_long %>% group_by(Latitude, Longitude, `Depth sed [m]`, Symbiosis, Spine) %>%
  summarise_all(.funs = sum, na.rm=T)  %>% ungroup()
core12_long <- core12_long %>% group_by(Latitude, Longitude, `Depth sed [m]`, Symbiosis, Spine) %>%
  summarise_all(.funs = sum, na.rm=T)  %>% ungroup()
core13_long <- core13_long %>% group_by(Latitude, Longitude, `Depth sed [m]`, Symbiosis, Spine) %>%
  summarise_all(.funs = sum, na.rm=T)  %>% ungroup()
core14_long <- core14_long %>% group_by(Latitude, Longitude, `Depth sed [m]`, Symbiosis, Spine) %>%
  summarise_all(.funs = sum, na.rm=T)  %>% ungroup()
core15_long <- core15_long %>% group_by(Latitude, Longitude, `Depth sed [m]`, Symbiosis, Spine) %>%
  summarise_all(.funs = sum, na.rm=T)  %>% ungroup()
core16_long <- core16_long %>% group_by(Latitude, Longitude, `Depth sed [m]`, Symbiosis, Spine) %>%
  summarise_all(.funs = sum, na.rm=T)  %>% ungroup()
core17_long <- core17_long %>% group_by(Latitude, Longitude, `Depth sed [m]`, Symbiosis, Spine) %>%
  summarise_all(.funs = sum, na.rm=T)  %>% ungroup()
