...
core1 <- core1 %>%
  ...
  select(-"Age [ka BP]") %>% mutate(Event="MD95-2042", .before=Latitude)
...
core2 <- core2 %>% dplyr::filter(`Depth sed [m]` > 3.415 & `Depth sed [m]` < 4.885) %>%
    mutate(Event="MD95-2040", .before=Latitude)
...
core3 <- core3 %>% dplyr::filter(`Depth sed [m]` == 0.6)%>%
    mutate(Event="V26-124", .before=Latitude)
...
core4 <- core4 %>% mutate(Latitude = -44.25, Longitude = 149.98, .before = "Depth sed [m]")%>%
    mutate(Event="Fr194-GC3", .before=Latitude)
...
core5 <- core5 %>% mutate(Latitude = -39.312830, Longitude = 142.683980, .before = "Depth sed [m]") %>%
    mutate(Event="SS0206-GC15", .before=Latitude)
...
core6 <- core6 %>% mutate(Latitude = 71.621800, Longitude = 22.926100, .before = "Depth sed [m]") %>%
    mutate(Event="JM05-085-GC", .before=Latitude)
...
core10 <- core10 %>% dplyr::filter(`Depth sed [m]` >= 1.1 & `Depth sed [m]` <= 1.23) %>%
    mutate(Event="PS2644-5", .before=Latitude)
...
core11 <- core11 %>% mutate(Latitude = 69.371800, Longitude = -6.553000, .before = "Depth sed [m]") %>%
    mutate(Event="PS1243-1", .before=Latitude)
## PS1244-2
...
core12 <- core12 %>% mutate(Latitude = 69.365200, Longitude = -8.657000, .before = "Depth sed [m]") %>%
    mutate(Event="PS1244-2", .before=Latitude)
## PS1246-2
...
core13 <- core13 %>% mutate(Latitude = 69.385300, Longitude = -12.874800, .before = "Depth sed [m]") %>%
    mutate(Event="PS1246-2", .before=Latitude)
## PS1906-2
...
core14 <- core14 %>% mutate(Latitude = 76.846300, Longitude = -2.150500, .before = "Depth sed [m]") %>%
    mutate(Event="PS1906-2", .before=Latitude)
## GIK17732-1
...
core15 <- core15 %>% mutate(Latitude = 71.613333, Longitude = 4.213333, .before = "Depth sed [m]") %>%
    mutate(Event="GIK17732-1", .before=Latitude)
## GIK23059-3
...
core16 <- core16 %>% mutate(Latitude = 70.305000, Longitude = -3.123333, .before = "Depth sed [m]") %>%
    mutate(Event="GIK23059-3", .before=Latitude)
## GIK15612-2
...
core17 <- core17 %>% mutate(Latitude = 44.360000, Longitude = -26.543333, .before = "Depth sed [m]") %>%
    mutate(Event="GIK15612-2", .before=Latitude)
...
core1_long <- core1 %>% pivot_longer(cols = -c("Event", "Latitude", "Longitude", "Depth sed [m]", ), names_to = "Species", values_to = "Absolute Abundance")
core2_long <- core2 %>% pivot_longer(cols = -c("Event", "Latitude", "Longitude", "Depth sed [m]"), names_to = "Species", values_to = "Absolute Abundance")

core3_long <- core3 %>% pivot_longer(cols = -c("Event", "Latitude", "Longitude", "Depth sed [m]"), names_to = "Species", values_to = "Absolute Abundance")

core4_long <- core4 %>%
  select(-c("Sample ID", "Depth top [m] (Foram sample depths)", "Age dated [ka]", "Depth bot [m] (Foram sample depths)")) %>%
  pivot_longer(cols = -c("Event", "Latitude", "Longitude", "Depth sed [m]", ), names_to = "Species", values_to = "Absolute Abundance")
...
core5_long <- core5 %>% pivot_longer(cols = -c("Event", "Latitude", "Longitude", "Depth sed [m]"), names_to = "Species", values_to = "Absolute Abundance")
...
core12_long <- core12 %>% pivot_longer(cols = -c("Event", "Latitude", "Longitude", "Depth sed [m]"), names_to = "Species", values_to = "Absolute Abundance")
core13_long <- core13 %>% pivot_longer(cols = -c("Event", "Latitude", "Longitude", "Depth sed [m]"), names_to = "Species", values_to = "Absolute Abundance")
core14_long <- core14 %>% pivot_longer(cols = -c("Event", "Latitude", "Longitude", "Depth sed [m]"), names_to = "Species", values_to = "Absolute Abundance")
core15_long <- core15 %>% pivot_longer(cols = -c("Event", "Latitude", "Longitude", "Depth sed [m]"), names_to = "Species", values_to = "Absolute Abundance")
core16_long <- core16 %>% pivot_longer(cols = -c("Event", "Latitude", "Longitude", "Depth sed [m]"), names_to = "Species", values_to = "Absolute Abundance")
core17_long <- core17 %>% pivot_longer(cols = -c("Event", "Latitude", "Longitude", "Depth sed [m]"), names_to = "Species", values_to = "Absolute Abundance")
